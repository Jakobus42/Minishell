...
/*   Updated: 2024/11/02 14:50:27 by lbaumeis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	set_env(t_shell *shell, t_list *env, char *key, char *value)
{
	...
	pair = make_new_pair(pair, shell, key, value);
	...
	while (env)
	{
		...
		if (!s)
			return (error_fatal(shell, "strdup in conv env to arr failed\n",
					MALLOC_FAIL), free_array((void ***)&converted_env), NULL);
		converted_env[i] = ft_strjoin_null(s, pair->value);
		free_and_null((void **)&s);
		if (!converted_env[i])
			return (free_array((void ***)&converted_env), error_fatal(shell,
					"strjoin in conv env to arr failed\n", MALLOC_FAIL), NULL);
		env = env->next;
		i++;
	}
